...
class DependencyFilterPanel:
    ...
    """
    def __init__(self, gui, nlp_canvas: Qt5NLPCanvas):

        _filter = nlp_canvas.filter

        def label_field_changed(text):
            _filter.allowed_labels.clear()
            split = text.split(',')
            for label in split:
                _filter.allowed_labels.add(label)
            nlp_canvas.update_nlp_graphics()

        def token_text_field_changed(text):
            _filter.allowed_token_propvals.clear()
            split = text.split(',')
            for tok_property in split:
                ...
                _filter.allowed_token_propvals.add(tok_property)
            nlp_canvas.update_nlp_graphics()

        def use_path_action(value):
            _filter.use_path = bool(value)
            nlp_canvas.update_nlp_graphics()

        def collapse_action(value):
            _filter.collapse = bool(value)
            nlp_canvas.update_nlp_graphics()

        def whole_words_action(value):
            _filter.propvals_whole_word = bool(value)
            nlp_canvas.update_nlp_graphics()
